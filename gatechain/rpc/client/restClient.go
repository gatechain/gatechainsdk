...
import (
	...
	nodetypes "github.com/gatechain/gatechainsdk/gatechain/types"
	"io"
	"io/ioutil"
	"net/http"
	"net/url"
...
func (client RestClient) GetPendingTransactions(maxTxns uint64) (response v1.ProxyPendingTransactions, err error) {
	err = client.get(&response, fmt.Sprintf("/transactions/pending"), pendingTransactionsParams{maxTxns})
	return
}

func (client RestClient) GetPendingNonce(addr string) (response uint64, err error) {
	err = client.get(&response, fmt.Sprintf("/transactions/pendingNonce/%s", addr), nil)
	return
}
...
//// BlockCert gets the block info for the given round
//func (client RestClient) BlockCert(round uint64) (response util.BlockCert, err error) {
//	err = client.get(&response, fmt.Sprintf("/blockcert/block/%d", round), nil)
//	return
//}
...
func (client RestClient) QueryAccount(addr string) (response nodetypes.RspAccount, err error) {
	path := fmt.Sprintf("/account/%s", addr)
