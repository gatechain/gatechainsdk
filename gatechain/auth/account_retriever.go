...
func NewAccountRetriever(querier NodeQuerier, cdc *codec.Codec) AccountRetriever {
	return AccountRetriever{querier: querier, cdc: cdc}
}
...
func (ar AccountRetriever) GetAccountWithHeight(addr sdk.AccAddress) (exported.Account, int64, error) {
	bs, err := ar.cdc.MarshalJSON(NewQueryAccountParams(addr))
	if err != nil {
		return nil, 0, err
	}
	...
	if err := ar.cdc.UnmarshalJSON(res, &account); err != nil {
		return nil, height, err
	}
...
func (ar AccountRetriever) GetAccountNumber(addr sdk.AccAddress) (uint64, error) {
