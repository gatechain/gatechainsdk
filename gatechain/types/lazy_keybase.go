...
func (lkb lazyKeybase) GetByAddress(address AccAddress) (Info, error) {
	db, err := sdk.NewLevelDB(lkb.name, lkb.dir)
	if err != nil {
		return nil, err
	}
...
func (lkb lazyKeybase) CreateEthAccount(name, mnemonic, bip39Passwd, encryptPasswd string, account uint32, index uint32) (Info, error) {
	db, err := sdk.NewLevelDB(lkb.name, lkb.dir)
	if err != nil {
		return nil, err
	}
	...
	return newDbKeybase(db).CreateEthAccount(name, mnemonic, bip39Passwd, encryptPasswd, account, index)
}
