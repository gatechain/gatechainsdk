...
func (coins Coins) IsValid() bool {
	...
	default:
		...
		upDenom := coins[0].Denom
		for _, coin := range coins[1:] {
			if strings.ToUpper(coin.Denom) != coin.Denom {
				return false
			}
			if coin.Denom <= upDenom {
				return false
			}
			if !coin.IsPositive() {
				return false
...
func (coins Coins) NanoToWei() Coins {
	var newCoins Coins
	for _, coin := range coins {
		if coin.Denom == DefaultBondDenom {
			coin.Denom = AttoPhoton
			coin.Amount = coin.Amount.Mul(NanoUnit)
		}
...
func ParseCoin(coinStr string) (coin Coin, err error) {
	...
	if matches == nil {
		return ParseSciCoin(coinStr) // gatechain add
	}
...
// ParseSciCoin parses a cli input for one coin type, returning errors if invalid.
// This returns an error on an empty string as well.
// gatechain add
func ParseSciCoin(coinStr string) (coin Coin, err error) {
	coinStr = strings.TrimSpace(coinStr)

	matches := reSciCoin.FindStringSubmatch(coinStr)
	if matches == nil {
		return Coin{}, fmt.Errorf("invalid coin expression: %s", coinStr)
	}

	denomStr, amountStr := matches[2], matches[1]
	amountFormat := amountStr
	amountDecimal, err := decimal.NewFromString(amountStr)
	if err == nil {
		amountFormat = amountDecimal.String()
	}

	amount, ok := NewIntFromString(amountFormat)
	if !ok {
		return Coin{}, fmt.Errorf("failed to parse coin amount: %s", amountStr)
	}
