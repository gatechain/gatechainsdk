...
if [ ! -d ${SODIUM_PATH}/${srcname} ]; then
    echo "start to download libsodium..."
    curl https://codeload.github.com/algorand/libsodium/zip/${srcpath} > ${SODIUM_PATH}/${libname}
    unzip ${libname}
fi

echo "start to build libsodium"
cd ${srcname} && \
		./autogen.sh && \
...
echo "build libsodium successfully"
